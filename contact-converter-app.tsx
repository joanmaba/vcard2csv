...
export default function ContactConverter() {
  ...
  const parseVCard = useCallback((vCardContent) => {
  ...
  const convertToCSV = useCallback(async () => {
    ...
    try {
      ...
      setError(`Error en processar els fitxers: ${err.message}`);
      setIsProcessing(false);
    }
  }, [files, parseVCard]);
    ...
    <div className="flex flex-col min-h-screen bg-gray-50">
      <header className="bg-blue-600 text-white py-4 shadow-md">
        <div className="container mx-auto px-4">
          ...
          <p className="text-blue-100">Converteix vCards i altres formats a CSV</p>
        </div>
      </header>
      ...
      <footer className="bg-gray-100 py-4 text-center text-gray-600 text-sm">
        <p>Convertidor de Contactes vCard a CSV © 2025</p>
      </footer>
    </div>
  );
}